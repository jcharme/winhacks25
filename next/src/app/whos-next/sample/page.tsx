...
export default function Page() {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    setExpenses([
      ...expenses,
      { who: transaction.email, amount: transaction.amount },
    ]);
  };
    ...
    <div className="grid grid-cols-10">
      <div id="user" className="col-span-3 h-screen bg-blue-100 p-2">
        ...
        <p className="block text-center">
          {owe < 0 ? "Your turn!" : "Their treat!"}
        </p>
        <BarChart balance={owe} />
      </div>
      <div
        id="expenses"
        className="col-start-4 col-span-7 h-screen bg-[#a2ccec]"
      >
        ...
        <ul className="list-none m-2">
          {expenses.map((e, i) => {
            return <Expense email={e.who} amount={e.amount} key={i} />;
          })}
        </ul>
      </div>
    </div>
